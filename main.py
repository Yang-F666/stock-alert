...
def check_alerts(code, name, market, df):
    ...
    if cost is not None:
        ...
        if pnl_pct > 0 and -25.0 < drop_from_high <= -15.0:
            alerts.append(
                f"📉 **减仓提醒（30%）**\n"
                f"当前盈利 {pnl_pct:+.2f}%，"
...
def process_stock(code, name, market):
    ...
    df = get_history_a(code) if market == "A股" else get_history_hk(code)
...
def main():
    ...
    print("BaoStock 登录...")
    lg = bs.login()
    print("BaoStock 登录成功" if lg.error_code == "0" else f"BaoStock 登录失败: {lg.error_msg}")
    ...
    if all_data:
        ...
        if failed_stocks:
            send_wechat(
                "⚠️ 数据获取异常",
                f"{today_str} 所有数据获取失败。\n失败：{'、'.join(failed_stocks)}"
            )
        else:
            send_wechat("股市休市通知", f"{today_str} 今日为非交易日。")
